...
| F02 | **SQLiteデータベース導入** | 過去論文の検索、履歴無制限化 | ファイルベースの限界。タグ/ブックマーク/メモの基盤 | 8人日 | **実装済み** ✅ |
| F03 | **PDF一括ダウンロード** | オフライン閲読、出張・移動時の必需品 | `arxiv` ライブラリの既存メソッド活用。低難易度 | 3人日 | **実装済み** ✅ |
| F04 | **タグ付け・ラベリング** | 「要精読」「サーベイ用」等の分類管理 | DB前提。`tags` + `paper_tags` テーブル設計 | 4人日 | **実装済み** ✅ |
| F05 | **BibTeX/RISエクスポート** | 論文執筆時の必須出力形式 | Paper dataclassから生成。低工数 | 2人日 | **実装済み** ✅ |
